...
# Usage:
#   bash run_incremental_contrib.sh \
#     --aipv_in_csv datasets/dataset_for_aipv.csv \
#     --sipv_in_csv outputs/AIPV_output.csv \
#     --out_dir outputs/incremental_contrib \
#     --runs 1 \
#     --threshold 0.5 \
#     --tau_sent 0.5

# Notes:
# - --aipv_in_csv is used for A-IPV incremental runs.
# - --sipv_in_csv is used for S-IPV incremental runs.
# - If --sipv_in_csv is not provided, S-IPV defaults to each run's A0 output (A-IPV full model).
...
for r in $(seq 1 "$RUNS"); do
  ...
  # -----------------------------
  # A-IPV decremental groups (full -> remove module)
  # A0: full model (validator + positive retrieval + hard negatives + iterative refine)
  # A1: remove iterative refine only (iteration comparison on full setting)
  # A2: remove hard negatives (on top of A1)
  # A3: remove positive retrieval (on top of A2)
  # A4: remove validator (on top of A3)
  # -----------------------------

  python3 AP_openai.py \
    --in_csv "$AIPV_IN_CSV" \
  ...
  # -----------------------------
  # S-IPV decremental groups (full -> remove module)
  # S0: full model (validator + iterative refine + memory)
  # S1: remove iterative refine only (iteration comparison on full setting)
  # S2: remove memory (on top of S1)
  # S3: remove validator (on top of S2)
  # -----------------------------

  python3 SP_openai.py \
