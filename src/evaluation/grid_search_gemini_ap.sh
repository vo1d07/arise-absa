...
# Gemini / Google grid search for A-IPV.
#
# Usage example:
#   bash grid_search_gemini_aipv.sh \
#     --in_csv datasets/dataset.csv \
#     --out_dir outputs/grid_search_gemini_aipv \
#     --runs 1 \
...
run_aipv() {
	if [[ -n "$GOOGLE_API_KEY" ]]; then
		GOOGLE_API_KEY="$GOOGLE_API_KEY" python3 AP_google.py "$@"
	else
...
echo "Gemini A-IPV grid search started"
