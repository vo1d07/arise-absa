...
# Claude / Anthropic grid search for S-IPV.
#
# Usage example:
#   bash grid_search_claude_sipv.sh \
#     --in_csv outputs/baseline_test_seed42_sipv.csv \
#     --out_dir outputs/grid_search_claude_sipv \
#     --runs 1 \
...
#   --anthropic_api_key <key>         : Optional override for env ANTHROPIC_API_KEY
...
run_sipv() {
  if [[ -n "$ANTHROPIC_API_KEY" ]]; then
    ANTHROPIC_API_KEY="$ANTHROPIC_API_KEY" python3 SP_anthropic.py "$@"
  else
    python3 SP_anthropic.py "$@"
  fi
}
...
echo "Claude S-IPV grid search started"
