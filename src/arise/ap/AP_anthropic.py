...
Prereqs:
    pip install pandas pyarrow numpy anthropic sentence-transformers tqdm

Env:
    export ANTHROPIC_API_KEY="..."

Example:
    python3 A-IPV_anthropic.py \
...
def parse_json_with_fallback(s: str) -> Any:
    ...
    obj_start = [i for i in (s.find("{"), s.find("[")) if i != -1]
    if not obj_start:
        raise
...
"""
    ...
    def _chat(self, user_content: str) -> str:
        resp = self.client.messages.create(
            ...
            system=sanitize_message_content(self._system()),
            messages=[{"role": "user", "content": sanitize_message_content(user_content)}],
        )
...
class Validator:
    ...
    def _chat(self, system: str, user: str) -> str:
        resp = self.client.messages.create(
            ...
            max_tokens=1500,
            temperature=self.temperature,
            system=sanitize_message_content(system),
            messages=[{"role": "user", "content": sanitize_message_content(user)}],
        )
        return anthropic_text_from_response(resp)
...
def main():
    ...
    ap.add_argument("--p_model", default="claude-haiku-4-5-20251001")
    ap.add_argument("--v_model", default="claude-haiku-4-5-20251001")
    ...
    if os.path.isdir(args.out_csv):
        args.out_csv = os.path.join(args.out_csv, "A-IPV_anthropic.csv")
