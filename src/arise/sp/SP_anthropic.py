...
Uses Anthropic Claude models for sentiment classification.
...
def anthropic_text_from_response(resp: Any) -> str:
    parts: List[str] = []
...
class LLMSentimentClassifier:
    """LLM-based sentiment classifier using Anthropic API."""
    
    def __init__(self, model: str = "claude-3-5-haiku-latest", temperature: float = 0.0, api_key: Optional[str] = None):
    ...
    def _chat(self, system: str, user: str) -> str:
        resp = self.client.messages.create(
            ...
            messages=[{"role": "user", "content": sanitize_text_for_api(user)}],
        )
        return anthropic_text_from_response(resp)
    ...
    def validate_evidence_consistency(self, aspect: str, label: str, evidence_phrases: List[str], 
        ...
        if not aspect_entries:
            ...
            diagnostics.update({
                ...
                "cold_start": True,
                "S_rels": S_rels,
                "llm_support": {k: float(v) for k, v in llm_probs.items()},
                "best_label": max(llm_probs.items(), key=lambda kv: kv[1])[0],
                "proposed_support": float(llm_probs[label]),
        ...
        retrieved_supports: Dict[str, List[Dict[str, Any]]] = {}
...
def load_models(models_dir: str, llm_model: str = "claude-3-5-haiku-latest", api_key: Optional[str] = None) -> SentimentModels:
...
def validator_validate(text: str, aspect: str, pred: Pred, memory: Memory, models: SentimentModels) -> Verdict:
    ...
    if isinstance(diagnostics, dict):
        ...
        llm_support = diagnostics.get("llm_support", {})
        proposed_support_raw = 0.0
        if isinstance(llm_support, dict):
            proposed_support_raw = llm_support.get(pred.label, 0.0)
        else:
            proposed_support_raw = diagnostics.get("proposed_support", 0.0)
        ...
        validator_feedback["llm_confidence"] = {
            ...
            "all_supports": {k: float(v) for k, v in llm_support.items()} if isinstance(llm_support, dict) else {},
        }
        ...
        if isinstance(llm_support, dict):
            llm_support_for_label = llm_support
...
def main():
    ...
    # LLM options
    ap.add_argument("--llm_model", default="claude-haiku-4-5-20251001", help="Anthropic model name (e.g., claude-3-5-haiku-latest, claude-3-7-sonnet-latest)")
    ap.add_argument("--anthropic_api_key", default=None, help="Anthropic API key (default: ANTHROPIC_API_KEY env var)")
